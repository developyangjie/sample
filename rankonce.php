...
function autoAct15Reward()
{
    $ret = sql_update("insert into umail (uid,mcontent,ucoin,itemid,count,ts,system) (select uid as uid,CONCAT('小掛從南瓜俠手上收到了一批獎勵,小夥伴們快來領取你的獎勵吧!(南瓜頭領*',act15touling,' 南瓜嘍嘍*',act15loulou,' 糖果*',act15tangguo,')') as mcontent,(act15loulou*20000+act15tangguo*50000) as ucoin,8 as itemid,100*act15touling as count,UNIX_TIMESTAMP() as ts, 1 as system from uact where act15touling>0 or act15loulou>0 or act15tangguo>0)");
    sql_update("update uact set act15touling=0,act15loulou=0,act15tangguo=0");
    cronLog("database " . DB_DATABASE . " ok|" . $ret . "\n");
}

function _rankonce_docvcbout()
{
    $w = 2;
    while ($w < 9) {
        cronLog("start docvcbout bout=" . $w);
        cronLog(" ");
        docvcbout($w);
        cronLog(" ");
        cronLog("end docvcbout bout=" . $w);
        $w = $w + 1;
        sleep(5);
    }
}
